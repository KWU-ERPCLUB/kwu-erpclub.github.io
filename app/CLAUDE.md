...
- **인사이트 개편(2026-09-11~12, spec = `erp-club/docs/specs/2026-09-11-인사이트-개편.md`)**: 기사 분류 = **축 3값**(AI활용·AI×취업·AI×MIS, `주제` 폐지·세미나만 유지) · `보관`(목록 제외·URL 유지) · `후보출처`(심층 = 주간 frontmatter `심층후보` ⓪~⑥ 전부 T 행만 — validate 2차가 교차 검사) · 새 규칙 글(date ≥ `NEW_RULES_FROM` 2026-09-12) = 이미지 필수·대시(—) 금지·분량(심층 1,800~3,000·주간 ≤2,500, 링크·출처 제외)·심층 6칸(`::: 질문` 3 + 무슨 일/실물/앞선 사례/판정 기준/확인 안 된 것/SO WHAT + 출처 5). 썸네일 = `이미지` 1계층(자동 커버 전부 삭제, 주간 = 이미지 + 주차 배지). Markdown 블록 `탑`·`브리핑`·`질문` 신설. 필터 = 축 칩 1줄 + 주간만(`?axis=`). 운영 원천 = `ops/주간-트렌드-절차.md`·`ops/인사이트-발행-절차.md`.

...
// 값 근거 = RECRUIT_GOAL_LEAD·RECRUIT_EXAMPLES.
export const AIM_HOOK = '리포트·시험·팀플에 AI를 제대로 붙이는 법을 익히고, 만든 도구를 학기 내내 쓴다.'
...
// 이 스터디가 하는 일(2026-08-27 오너 3차 개정 — "역량·순서가 두루뭉술" → 장면+도구로 구체화, 산출물·운영에 쓰는 것 섹션 삭제).
// 멤버 표면 = 취업용 AI 역량(북극성 §3). 내부 운영 프레임(AX 도입 시뮬레이션)은 웹 비표기 — 오너 전용.
// 구조 = 리드 1줄 → 예시 5(카드: 장면 제목 + 실제 하는 일 + 도구) → 한 학기 흐름 3(번호).
export const RECRUIT_GOAL_LEAD = '기업이 말하는 AI 역량을, 내 과제·시험·팀플에서 직접 만든다.'

// 예시 5 = [장면 제목, 실제로 하는 일, 도구]. 각 항목이 역량 5(쪼갠다·기준·검수·도구·자산)에 1:1 대응 — 역량 이름은 웹에 안 쓴다.
export const RECRUIT_EXAMPLES = [
  ['리포트 — 채점 기준을 먼저 붙인다', '과제 안내문에서 평가 기준을 뽑아 AI에 고정하고, 초안을 그 기준으로 자기 채점시킨 뒤 고쳐 낸다.', 'Gemini Gems · Claude'],
  ['시험 — 강의자료를 넣어 둔 노트북에 묻는다', '강의자료·내 필기를 통째로 넣고, 출처가 달린 요약·예상 문제·퀴즈를 뽑아 복습한다.', 'NotebookLM'],
  ['검수 — 다른 AI에게 틀린 곳을 찾게 한다', '한 AI가 쓴 초안을 다른 AI에 넘겨 출처 확인·반박을 시킨다. 첫 답을 그대로 쓰지 않는다.', 'Gemini ↔ Claude/GPT'],
  ['팀플 — 회의 녹취를 결정과 할 일로 바꾼다', '회의 녹음을 넣으면 결정·담당·기한 표와 공지 메일 초안이 나온다. 회의록을 사람이 쓰지 않는다.', 'Gemini + Gmail·Docs'],
  ['재사용 — 한 번 만든 걸 미니앱으로 남긴다', '잘 된 지시문을 버튼 하나짜리 미니앱으로 만들어 다음 과제와 동료에게 그대로 쓴다.', 'Opal · Gems 공유'],
]

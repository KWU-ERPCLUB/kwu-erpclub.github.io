...
// 값 근거 = RECRUIT_GOAL_LEAD·RECRUIT_FLOW.
export const AIM_HOOK = '과제·시험·팀플·발표에 AI를 붙여, 기업이 말하는 AI 역량을 본인 사례로 만든다.'
...
// 이 스터디가 하는 일(2026-08-27 오너 개정 — 구 활동 4카드 폐지: "뜬구름·와닿지 않음").
// 멤버 표면 = 취업용 AI 역량(북극성 §3 유효 논거). 구조 = 리드 1줄 → 역량 5(체크) → 하는 일 6(순서) → 산출물 2.
// 내부 운영 프레임(AX 도입 시뮬레이션)은 웹 비표기 — 오너 전용.
export const RECRUIT_GOAL_LEAD = '기업이 말하는 AI 역량을 한 학기 동안 본인 사례로 만든다.'

// 역량 5 = 학습정의 C1~C7의 멤버 표면 압축(체크리스트 문법 = rc-who 승계).
export const RECRUIT_SKILLS = [
  ['업무를 쪼갠다', 'AI에 시킬 수 있는 단위로 내 일을 나눈다.'],
  ['기준을 붙여 시킨다', '질문 한 줄이 아니라 기준·자료를 붙여 위임한다.'],
  ['결과를 검수한다', '출처 확인·반증·교차 대조로 틀린 것을 잡는다.'],
  ['도구를 고른다', '일에 맞는 도구를 비교해 배치하고 비용을 판단한다.'],
  ['자산으로 남긴다', '한 번 만든 지시문·노트북·미니앱을 다음 과제에 다시 쓴다.'],
]

// 하는 일 = 도입 순서 그대로(번호 흐름).
export const RECRUIT_FLOW = [
  ['업무 목록', '한 학기 동안 반복하는 일을 전부 적는다. 리포트, 시험 대비, 팀플 회의, 발표, 메일.'],
  ['대체 구간 찾기', '그중 AI에 시킬 수 있는 단위를 고른다. 왜 지금까지 안 시켰는지도 적는다.'],
  ['도구 붙이기', '무료·학생 혜택 도구(Gemini, NotebookLM, Copilot, Claude/GPT)를 업무별로 배치한다. 어떤 일에 어떤 도구가 맞는지 직접 비교한다.'],
  ['검수·개선', '결과를 기준표로 검수하고, 안 되는 지점을 고쳐 다시 시킨다.'],
  ['자산으로 남기기', '지시문·노트북·미니앱을 다음 과제에도 쓴다.'],
  ['팀 케이스', '2차 구간. 같은 과정을 가상 업무 케이스에 팀으로 적용한다. 기획은 사람, 구현은 AI.'],
]

// 산출물 2(개인·팀) — rc-fit 카드 문법 승계.
export const RECRUIT_OUTPUT = [
  ['개인', '내 업무 AI 도입 기록 1부. 업무 목록, 도입 전후, 도구 배치, 지시문, 미니앱.'],
  ['팀', '업무 케이스 자동화 실물 + 결정 메모 1장. 발표 자료는 만들지 않는다.'],
]
...
// 2026-08-27 오너: 웹 썸네일 3장이 "웹디자인 스터디"로 읽힘 → 썸네일 폐지·텍스트 링크 2건(보드 카드 삭제). 용도 = 운영진이 AI로 운영하는 방식의 예시.
export const SHOWCASE_LEAD = '운영진이 AI로 만들어 운영에 쓰는 것. 사이트 제작은 커리큘럼에 없다.'
// 카드 문안 2단: core = 핵심 한 문장 / sub = 한 줄 보조. 대시·부연 금지, 한 줄 안에 끝낸다(오너 2026-08-15).
export const RECRUIT_SHOWCASE = [
  { name: 'AIM 허브 사이트', core: '인사이트·워크스페이스·모집이 도는 지면', sub: '운영 도구. 만드는 법은 다루지 않는다', href: '/projects/site/' },
  { name: 'AIM 특강 자료', core: '질문에서 위임으로', sub: '발표자료 45장', href: '/seminars/?p=2026-07-25-bapzzi-question-to-delegation' },
]

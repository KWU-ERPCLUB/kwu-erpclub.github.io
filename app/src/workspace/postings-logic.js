...
// 공고 행 계약: { 제목, 종류('공모전'|'채용'|'자격증'|'교내활동'), 주최, url, 출처?, 접수시작?, 접수마감?, 시험일?, 코멘트?, 고정 }
// '자격시험' → '자격증' 개명(2026-08-18 오너) — DB 정합 = migration 0015(기존 행 update 포함).

// '대외활동' → '교내활동' 개명(2026-08-19 오너) — AIM 자체가 대외활동이라 외부 대외활동은 모으지 않는다.
// 이 자리는 광운대 교내 정보(비교과·특강·튜터·장학) 전용. DB 정합 = migration 0019.
export const POSTING_KINDS = ['공모전', '채용', '자격증', '교내활동']

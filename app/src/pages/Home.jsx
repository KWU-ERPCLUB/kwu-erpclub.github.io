  ...
  return (
    <>
      ...
      <main className="hub-page">
        ...
        <section aria-label="스터디 현황" className="hub-stats">
          <div><strong>{active}</strong> <span className="hub-meta">active 트랙</span></div>
          <div><strong>{handedOff}</strong> <span className="hub-meta">이양완료 트랙</span></div>
          <div><strong>{seminarCount}</strong> <span className="hub-meta">세미나 회차 기록</span></div>
        </section>

        <section className="hub-section">
          <h2 className="hub-h2">트랙</h2>
            ...
            {tracks.map((t) => (
              <li key={t.id} className="hub-card">
                <div className="hub-card-inner">
                  ...
                  {t.tool ? <a className="proof-link" href={t.tool}>도구 열기 →</a> : <span className="hub-meta">준비</span>}
                </div>
              </li>
            ))}
          </ul>

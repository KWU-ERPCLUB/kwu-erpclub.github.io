...
test('메인 = 정중앙 히어로(대형 타이포·블롭 배경) — 마퀴·스크롤 유도 부재', () => {
  ...
  // 히어로: 뷰포트 타이포(hero-mega) + 문자 스태거 리빌(sc) + 브랜드 키커 + 추상 블롭 배경
  ...
  // 확장 라인(AI × MIS) — AIM 폭 양끝 정렬(.hero-brand 묶음)
  ...
  // 4차 폐지분: 마퀴 띠·스크롤 유도·우측 격자 키비주얼
  expect(html).not.toContain('marquee')
  expect(html).not.toContain('scroll-cue')
  expect(html).not.toContain('hero-kv')
})

test('섹션 헤드 4종(중앙 정렬) + 각 섹션 구조 마크업', () => {
  const html = renderToString(<App />)
  for (const idx of ['ROADMAP', 'PROJECTS', 'FAQ', 'INSIGHTS']) {
    expect(html).toContain(`>${idx}<`)
  }
  // 4차: 좌 라벨 컬럼(hs-cols) 폐지 → 중앙 섹션 헤드(hs-head/hs-label) 4회
  ...
  // PROJECTS: 2열 커버 카드 + /projects/ 딥링크
  ...
  // FAQ: 아코디언(네이티브 details/summary) + WHY 이관 1문(챗 단독의 한계)
...
test('메인 FAQ = data/faq.js 원천 전량 렌더', () => {
  const html = renderToString(<App />)
  expect(FAQ.length).toBe(10) // 증보 4문항(시간·학점·개인정보·전공) 포함 2026-08-05 2차

...
이 이슈가 우리에게 닿는 경로부터 정확히 하자. 실제 회사에서 이 다섯 질문의 설계 주체는 정보보호·IAM(계정권한관리) 조직이다. 경영·기획 직무는 "이 업무엔 어떤 권한이 필요하고 예외는 누가 승인하나"라는 요건을 주는 쪽이고, 신입이 이 설계를 맡는 일은 없다. MIS가 배우는 내부통제와 겹치는 것은 설계 권한이 아니라 *요건을 말하는 언어*다.

Uber처럼 "통제를 먼저 깔고 도입"하는 그림은 자체 플랫폼 팀을 가진 회사 얘기다. 국내 SI 프로젝트는 납기·예산 구조상 통제가 감리·보안점검 단계에서 뒤늦게 붙는 경우가 많다 — 입사 후 마주칠 현실은 전자보다 후자에 가깝다.

지금 할 수 있는 것은 규모를 줄인 같은 문제다. 스터디에서 에이전트에 권한을 붙일 때 위 표의 축소판(무슨 권한을 줬나 · 기록이 남나 · 되돌릴 수 없는 행동 앞에 승인이 있나)을 직접 적용해 보고, Replit 사고 기록 원문을 한 번 읽어 두는 것. 2026년 1월 22일 시행된 **AI 기본법**이 고영향 AI에 투명성·영향평가 의무를 걸었으므로, 이 언어는 준수 요건의 언어이기도 하다.

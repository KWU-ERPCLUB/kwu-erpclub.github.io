...
- 각 축 끝의 "시사점"과 마지막 "SO WHAT"에 경영학부·MIS·ERP 관점을 정리했다.
:::
...
상반기 흐름이 광운대 경영학부·MIS 학부생의 현실에 닿는 거리를 재면 이렇다.

먼저 시차. SAP의 자율 기업·Joule은 S/4HANA(특히 클라우드) 전제인데, 국내 SAP 고객 상당수는 아직 ECC거나 온프렘이다. "ERP에 에이전트가 붙는다"가 국내 실무 현장의 기본값이 되기까지는 전환 프로젝트라는 긴 다리가 남아 있다 — 뒤집으면 그 전환 프로젝트가 앞으로 몇 년간 SI·컨설팅 인력이 실제로 하는 일이라는 뜻이다.

솔직하게 그어야 할 선도 있다. 신입 채용 6% 감소는 나쁜 소식이고, 그 이상도 이하도 아니다 — 자리가 줄면 경쟁은 세진다. "AI 거버넌스 문해력"도 신입 직무기술서에 들어가는 항목이 아니다. 실제 소관은 법무·컴플라이언스·정보보호 조직이고, 학부생에게는 차별화 무기가 아니라 면접에서 대화가 되는 수준의 배경지식이다.

그 전제 위에서 지금 할 수 있는 것: 화면을 직접 조작하는 에이전트(CUA)는 API 없는 낡은 전산이 많은 국내 기업에서 수요가 클 수밖에 없는 축이니, 이 계열의 도구를 직접 돌려 본 기록을 남겨 두는 것. 그리고 반도체 실적·정부 AI 예산이 만드는 채용·인턴·공모전은 뉴스보다 계획 문서(실행과제 99개)에 먼저 찍히므로, 지원 시즌 전에 그 문서를 확인하는 것. 둘 다 비용이 들지 않는다.
